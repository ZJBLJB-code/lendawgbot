...
# into tomcash/data/quant_bot_journal/ so CI can build without needing the
# full quant-bot worktree.
#
# Usage:
#   bash tomcash/scripts/sync_quant_bot.sh
#   QB_ROOT=/path/to/quant-bot bash tomcash/scripts/sync_quant_bot.sh
#
# This is the manual half of "Option A" data sync (see tomcash/README.md
...
# Live event logs — only the latest 7 to keep the repo small.
if compgen -G "$QB_ROOT/journal/live/*.jsonl" > /dev/null; then
...
echo "next: git add tomcash/data/quant_bot_journal && git commit"
